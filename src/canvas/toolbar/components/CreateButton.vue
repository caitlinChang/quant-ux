      ...
      <div class="MatcCreateBtnCntr MatcPadding">
        <div class="container-fluid">
          <div class="row" v-show="tab === 'widgets'">
            ...
            <div v-if="false" class="col-md-2 MatcCreateBtnLeft">
              <div class="form-group has-feedback">
                ...
                />
                <span
                  class="mdi mdi-magnify form-control-feedback MatcCreateSearchBtn"
                  aria-hidden="true"
          ...
          <div class="row" v-show="tab === 'import'">
            <div
              @mousedown.stop=""
              class="col-md-12 MatcCreateBtnElementList MatcCreateBtnRight"
            >
              <div data-dojo-attach-point="importCntr">
                <div class="MatcHint">Loading Apps...</div>
              </div>
              <div class="MatcButtonBar">
                <a class="MatcButton" @click="onSaveImports">Save</a>
                <a class="MatcLinkButton" @click="showWidgets()">Back</a>
              </div>
            </div>
          </div>
        </div>
  ...
  methods: {
      ...
      for (let i = 0; i < themes.length; i++) {
        ...
        if (theme.id) {
          ...
          if (this.categoriesList.indexOf(category) === -1) {
            console.warn(
              "CreateButton.onThemesLoaded() > Found new category. Consinder updating the CreateButton.vue componenet",
              category
            );
          ...
          if (!categories[category][theme.id]) {
            ...
            console.warn(
              "We have already a theme with the id",
              theme.id,
              theme
            );
          }
      ...
      for (let id in temp) {
        ...
        if (theme._extends) {
          ...
          if (parent) {
            ...
            if (parent._preview) {
              theme._preview = lang.mixin(
                lang.clone(parent._preview),
                theme._preview
              );
            }
            if (theme.error) {
            ...
            if (theme.checked) {
              theme.checked = lang.mixin(
                lang.clone(parent.checked),
                theme.checked
              );
            }
    ...
    render(categories) {
      ...
      for (let i = 0; i < cats.length; i++) {
        ...
        this.own(
          on(li, touch.press, lang.hitch(this, "showCategory", category, true))
        );
      }
      ...
      if (sharedLibs) {
        ...
        importApps.forEach((appHash) => {
          ...
          Services.getModelService()
            .findAppByHash(appHash)
            .then((app) => {
              this.own(
                on(
                  sharedLi,
                  touch.press,
                  lang.hitch(this, "showImportedApp", app.id, true)
                )
              );
            ...
            .catch((err) => {
              ...
              console.debug(
                "Createbutton.render() >  Cannot import app",
                appHash,
                err
              );
            });
      ...
      this.own(
        on(liTemplates, touch.press, lang.hitch(this, "showTemplates", true))
      );
          ...
          if (importedApp) {
            ...
            this.own(
              on(
                liImport,
                touch.press,
                lang.hitch(this, "showImportedApp", appID, true)
              )
            );
            Services.getModelService()
              .findApp(appID)
            ...
            console.warn(
              "Createbutton.render() > render() No access to ",
              appID
            );
          }
        });
      ...
      db.a("MatcButton MatcButtonFullWidth MatcButtonSignUp", "Import").build(
        li
      );
      this._lis["Import"] = li;
    ...
    renderIcons(query) {
      ...
      for (let j = 0; j < icons.length; j++) {
        ...
        this.tempOwn(
          on(span, touch.press, lang.hitch(this, "onCreateIcon", icon))
        );
      }
      if (!query) {
    ...
    renderElements(elements, category, isTemplate, append) {
      ...
      if (elements.length === 0) {
        if (isTemplate) {
          ...
          db.span(
            "MatcHint",
            "No symbols defined. To create a symbol, select a widget and click 'Create Component' in the toolbar!"
          ).build(cntr);
        ...
        for (let i = 0; i < elements.length; i++) {
          ...
          if (isTemplate) {
            let delBtn = db
              .div("MatcCreateBtnRemove  mdi mdi-close-circle")
              .build(div);
            this.tempOwn(
              on(
                delBtn,
                touch.press,
                lang.hitch(this, "onRemoveTemplate", child)
              )
            );
          }
        }
    ...
    renderScreenAndWidget(app, preview, db, size, isTemplate, elementDiv) {
      this.tempOwn(
        on(elementDiv, touch.press, lang.hitch(this, "onCreate", app))
      );
      ...
      if (screens.length === 1) {
        ...
        let centeredBox = this._createCenteredBox(
          db,
          preview,
          scalledScreen,
          size
    ...
    renderGroup(group, preview, db, size, isTemplate, elementDiv) {
      ...
      if (isTemplate) {
        this.tempOwn(
          on(elementDiv, touch.press, lang.hitch(this, "onCreate", group))
        );
        ...
        for (let i = 0; i < children.length; i++) {
           ...
           */
          templateChild = ModelUtil.inlineTemplateVariant(
            templateChild,
            this.model
          );
